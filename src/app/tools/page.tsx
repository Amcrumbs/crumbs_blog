      ...
      <div className="grid gap-5">
        {groups.map((group) => (
          <section key={group.category} className="surface p-5">
            <h2 className="font-mono text-sm text-[var(--green)]">{group.category}</h2>
            <div className="mt-4 grid gap-3 md:grid-cols-2 xl:grid-cols-3">
              {group.tools.map((tool) => {
                const localized = localizedTool(tool, locale);

                return (
                  <Link key={localized.slug} href={`/tools/${localized.slug}`} className="surface-strong group block p-4 hover:border-[var(--cyan)]">
                    <div className="flex items-start justify-between gap-3">
                      <div>
                        <h3 className="text-lg font-medium text-white">{localized.titleText}</h3>
                    ...
                    <p className="mt-4 font-mono text-xs text-[var(--amber)]">{t.tools.status[localized.status]}</p>
                  </Link>
                );
              })}
            </div>
